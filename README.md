...
2. Select the destination parent folder then the original folder will be moved inside it.
...
1. `Directory.Move(source, destination)` which relocates the folder
2. `Directory.CreateSymbolicLink(source, destination)` which creates a symlink at the original path

Any application referencing the original path will follow the symlink transparently. This only applies sometimes, some applications struggle
with symlinks so check documentation for those applications if you're having an issue.
